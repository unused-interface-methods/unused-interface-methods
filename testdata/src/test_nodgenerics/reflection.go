...
// ===============================
// ПРИМЕРЫ С РЕФЛЕКСИЕЙ (БЕЗ ДЖЕНЕРИКОВ)
// ===============================
...
// Кейс 31: Обычный интерфейс с рефлексией (вместо дженерика)
type SimpleReflectable interface {
	ReflectType() reflect.Type           // want "method \"ReflectType\" of interface \"SimpleReflectable\" is declared but not used"
	GetDefault() interface{}             // используется прямым вызовом
	ProcessReflected(v interface{}) bool // want "method \"ProcessReflected\" of interface \"SimpleReflectable\" is declared but not used"
}
...
type SimpleReflectableImpl struct {
	value interface{}
}

func (s *SimpleReflectableImpl) ReflectType() reflect.Type           { return reflect.TypeOf(s.value) }
func (s *SimpleReflectableImpl) GetDefault() interface{}             { return nil }
func (s *SimpleReflectableImpl) ProcessReflected(v interface{}) bool { return true }
...
func UseReflection() {
	...
	// Кейс 31: Простой интерфейс с рефлексией
	simpleImpl := &SimpleReflectableImpl{value: "test"}

	// Используем ReflectType через рефлексию
	sv := reflect.ValueOf(simpleImpl)
	reflectTypeMethod := sv.MethodByName("ReflectType")
	...
	defaultValue := simpleImpl.GetDefault()
	_ = defaultValue
}
