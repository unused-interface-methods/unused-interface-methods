...
type EventHandler interface {
	...
	OnError(handler func(error) bool) error              // не используется
	Subscribe(filter func(string) bool, cb func()) error // не используется
}

// Кейс 5: Интерфейс с каналами
type ChannelProcessor interface {
	SendData(ch chan<- string) error    // используется
	ReceiveData(ch <-chan string) error // не используется
	ProcessStream(ch chan string) error // не используется
}

// Кейс 6: Интерфейс с мапами и слайсами
type DataProcessor interface {
	ProcessMap(data map[string]interface{}) error // используется
	ProcessSlice(data []string) error             // не используется
	ProcessArray(data [10]int) error              // не используется
}

// Кейс 7: Интерфейс с указателями
type PointerHandler interface {
	HandlePointer(data *string) error // используется
	HandleDoublePtr(data **int) error // не используется
}
...
type NamedReturns interface {
	...
	GetMultipleNamed() (x, y int, success bool, err error) // не используется
}
...
type InterfaceParams interface {
	...
	HandleWriter(w io.Writer) error        // не используется
	HandleBoth(rw io.ReadWriter) error     // не используется
	HandleCustom(custom interface{}) error // не используется
}
...
type AnotherReader interface {
	CustomRead(data []byte) error // не используется, но имеет такое же имя как Reader.CustomRead
}
...
type AnyHandler interface {
	Process(data interface{}, options interface{}) error // не используется, но имеет такое же имя
}
